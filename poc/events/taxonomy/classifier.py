...
from .rules import ATTRIBUTE_RULES,CATEGORY_RULES,SUBCATEGORY_RULES,TAG_RULES
VALID=set(CATEGORY_RULES)|{"nightlife","community","other"}
@dataclass(frozen=True, slots=True)
class TaxonomyResult:
    category:str; subcategories:tuple[str,...]; attributes:tuple[str,...]; confidence:float; evidence:tuple[str,...]
    tags:tuple[str,...]=(); taxonomy_version:str="tax2"
def _hits(text,phrases): return [phrase for phrase in phrases if phrase in text]
def classify_many(candidates):
    ...
    hints=tuple(dict.fromkeys(h.lower().strip() for item in rows for h in item.category_hints)); scores={}; evidence=[]
    for hint in hints:
    ...
    if any(word in titles for word in ("museum","gallery","theatre","theater")): scores["arts_culture"]=scores.get("arts_culture",0)+3
    category=max(scores,key=lambda key:(scores[key],key)) if scores else "community"
    confidence=min(.98,.55+.08*scores.get(category,0)) if scores else .45; combined=titles+" "+descriptions
    ...
    domain_tags=tuple(name for name,phrases in TAG_RULES.items() if _hits(" "+combined+" ",phrases))
    tags=tuple(dict.fromkeys(attrs+domain_tags))
    return TaxonomyResult(category,subs,attrs,round(confidence,2),tuple(dict.fromkeys(evidence)),tags)
def classify(candidate): return classify_many((candidate,))
